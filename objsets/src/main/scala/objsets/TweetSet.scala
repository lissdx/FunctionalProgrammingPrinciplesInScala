...
class Tweet(val user: String, val text: String, val retweets: Int) {
  override def toString: String =
    //"~U:_" + user + "_T:_'" + text + "'_R:" + retweets + "~"
    "~" + text + "'_R:" + retweets + "~"
}
...
class Empty extends TweetSet {
  ...
  override def union(set: TweetSet): TweetSet = set
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  override def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    val newTweetSet = if (p(elem))
      acc.incl(elem)
    else
      acc

    (left.filterAcc(p, newTweetSet) union right.filterAcc(p, newTweetSet))
  }

  override def mostRetweeted: Tweet = {
    val leftMR = if( !left.isInstanceOf[ Empty ])
      left.mostRetweeted
    else
      elem

    val rightMR = if( !right.isInstanceOf[ Empty ])
      right.mostRetweeted
    else
      elem
  ...
  override def descendingByRetweet: TweetList = {
    val mrTweet = this.mostRetweeted
    new Cons( mrTweet, this.remove( mrTweet ).descendingByRetweet)
  }
  /**
   * The following methods are already implemented
   */
